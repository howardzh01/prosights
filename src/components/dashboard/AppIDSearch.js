...
export default function AppIDSearch({
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch("/api/private/getAppIDMappingResults", {
          method: "POST",
          headers: {
        ...
        console.log("Failed to fetch data ai mappings:", error);
        setOptions([]);
      } finally {
        setLoading(false);
      }
      ...
      <Autocomplete
        ...
        getOptionLabel={(option) =>
          typeof option === "string"
            ? option
            : `${option.UNIFIED_PRODUCT_NAME} - ${option.UNIFIED_PRODUCT_KEY}`
        }
        ...
        renderOption={(props, option, { selected }) => (
          <Box component="li" {...props}>
            ...
            <span className="text-sm text-customGray-800">
              <strong>{option.UNIFIED_PRODUCT_NAME}</strong> -{" "}
              {option.UNIFIED_PRODUCT_KEY}
            </span>
          </Box>
          // <Box component="li" {...props}>
